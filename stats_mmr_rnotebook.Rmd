...
Baseline model with just health expenditure and mmr
```{r}
baseline <- lm(mmr_transform ~ govt_health_transform + private_health_transform, data = data_after_mice)
summary(baseline)
```

Baseline model with added region fixed effect.
```{r}
...
Model including fixed effects shown important by prior literature.
```{r}
