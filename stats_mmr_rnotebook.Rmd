...
```
```{r}
p <- ggplot(data=data_after_mice, aes(x=log(govt_health_exp), y=mmr_transform, color = region)) +
  geom_point()+
  geom_smooth(method=lm, se=F)

p + scale_color_brewer(palette = "Paired") + xlab("Log of Government Health Expenditure") + ylab("Log of MMR")
```
...
```{r}
final_reg <- lm(mmr_transform ~ govt_health_exp + GDP + region + govt_exp_by_group + fertility_rate_transform + SAB + female_pop_transform + num_physicians, data = final_df)
summary(final_reg)
```